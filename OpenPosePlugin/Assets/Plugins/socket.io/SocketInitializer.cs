    ...
    public class SocketInitializer : MonoBehaviour {
        ...
        IEnumerator InitCore(UniRx.IObserver<Socket> observer, UniRx.CancellationToken cancelToken) {
            ...
            if (webSocketTrigger == null || !webSocketTrigger.IsConnected) {
                ...
                //// workaround
                //string textoCorrigido = www.text.Remove(www.text.Length - 4, 4);
                //textoCorrigido = textoCorrigido.Remove(0, 4); //www.text

                //var textIndex = textoCorrigido.IndexOf('{'); //www.text 

                //if (textIndex != -1) {

                // backend Paulo
                //string textoCorrigido = www.text.Remove(www.text.Length - 4, 4);
                //textoCorrigido = textoCorrigido.Remove(0, 4);

                var textIndex = www.text.IndexOf('{'); //www.text 
                if (textIndex != -1)
                { 
                    var json = www.text.Substring(textIndex); // www.text
